...
class LikeModelTestCase(TestCase):
    ...
    def test_like_model(self):
        ...
        l = Like(user_being_liked_id =self.uid1, user_liking_id=self.uid2)

        db.session.add(l)
        db.session.commit()

        # User1 should have 1 liker
        self.assertEqual(len(self.u1.likers),1 )
        self.assertEqual(len(self.u1.liking),0)
        self.assertEqual(len(self.u2.likers), 0)
        self.assertEqual(len(self.u2.liking),1)
